export default [
  [
    ['Bonjour voyageur. Bienvenue en Sorbonne', 0],
    ['Vous avez aujourd\'hui la chance de pouvoir', 3000],
    ['découvrir l\'étonnante histoire de ce célèbre batiment.', 6000],
    ['', 10000]
  ],
  [
    ['Parmi les milliers d’étudiants ayant fréquenté la Sorbonne,', 0],
    ['Richelieu est l’un des plus connus.', 3000],
    ['Célèbre ministre de Louis XIII,', 6000],
    ['homme religieux, intelligent, ambitieux,', 8000],
    ['Armand Jean du Plessis de Richelieu', 11000],
    ['naquit en 1585 au sein d’une grande fratrie.', 13500],
    ['Promis aux armes,', 18300],
    ['il assuma finalement la charge d’évêché de Luçon.', 19400],
    ['Homme de responsabilités,', 22900],
    ['le cardinal fut aussi proviseur de la Sorbonne,', 24600],
    ['des années plus tard...', 27500],
    ['Sa profonde volonté de renforcer le pouvoir royal', 29600],
    ['engendra un regain d’autorité,', 32600],
    ['appelé aujourd’hui “l’absolutisme".', 34500],
    ['Voici notre Richelieu jeune, en 1606,', 37300],
    ['lors de son arrivée en Sorbonne,', 41100],
    ['devant les portes de la chapelle médiévale..', 42700],
    ['Jetez donc un coup d’œil à ces vitraux', 46300], // 0
    ['qui, pour une fois, n’illustrent pas la Bible.', 48400],
    ['Ils racontent une toute autre histoire,', 51800],
    ['particulièrement rocambolesque...', 54200],
    ['Celle… d’un crâne.', 56800],
    ['Un crâne d’importance…', 59300],
    ['', 62000]
  ],
  [
    ['Mais… pourquoi tant d’agitation ?', 0],
    ['Ces gens ne ressemblent ni à des religieux, ', 2300],
    ['ni à des étudiants venus se recueillir auprès du tombeau !', 4300],
    ['Alors qui sont-ils ?', 7000],
    ['Le jeune Richelieu ne peut l’imaginer,', 8800],
    ['mais vous les aurez sûrement reconnus…', 11600],
    ['ce sont les manifestants populaires de 1789 :', 13500],
    ['les sans-culottes !', 17800],
    ['Ils sont venus pour profaner le tombeau.', 20000],
    ['Mais à qui appartient-il ?', 22000],
    ['Qui pourrait mériter un tel sort... ?', 24500],
	  ['', 28000]
  ],
  [
    ['Certains racontent que ce pauvre crâne', 0],
    ['trouva rapidement de nouveaux propriétaires :', 2000],
    ['de jeunes enfants, qui s’en seraient servi comme une balle de jeu !', 5000],
    ['Un abbé, passant par là, se serait mêlé à leur récréation.', 9000],
    ['', 13000]
  ],
  [
    ['Il en aurait même profité pour s’emparer de la tête,', 0],
    ['une fois ses jeunes amis lassés de la relique.', 3000],
    ['Le vol commis par l’abbé Baschamp', 5900],
    ['est envisageable, mais, rassurez-vous,', 8000],
    ['le récit du crâne devenu balle de jeu tient davantage de la légende !', 10200],
    ['Pourtant, cette légende fut racontée tant de fois', 14500],
    ['que beaucoup y crurent pendant des années !', 17000],
	  ['', 19500]
  ],
  [
    ['Une autre version de l’histoire existe.', 0],
    ['Un habitant du quartier aurait assisté au lugubre spectacle de la profanation.', 2700],
    ['Horrifié, il aurait alors récupéré,', 6900],
    ['puis protégé le crâne d’un morceau de linceul…', 9300],
    ['En vérité, il s’agissait d’un bonnetier appelé Cheval,', 12800],
    ['habitant à quelques rues de la Sorbonne.', 16700],
    ['Il est ainsi parvenu à mettre le crâne à l’abri,', 19500],
    ['et lui évita un destin funeste...', 21900],
	  ['', 24500]
  ],
  [
    ['Il semblerait que nous ayons quitté Paris !', 0],
    ['Allons-nous suivre les péripéties du crâne de ville en ville ?', 3200],
    ['Une fois en possession du bonnetier Cheval,', 7200],
    ['la relique sembla disparaître totalement.', 9300],
    ['Beaucoup la pensaient perdue, d’autres la disaient détruite.', 12000],
    ['Mais en réalité,', 16200],
    ['le crâne voyagea bien loin de Paris et de la Sorbonne.', 17800],
    ['Il alla même jusqu’en Bretagne !', 21800],
	  ['', 24500]
  ],
  [
    ['Mis en lieu sûr par le bonnetier Cheval,', 0],
    ['il fut par la suite confié aux mains du prêtre Nicolas Armez et de sa famille, dès 1805.', 2800],
    ['Transmis de génération en génération,', 9500],
    ['la famille du prêtre préserva ainsi la relique durant près de 60 ans !', 12000],
	  ['', 17500]
  ],
  [
    ['Un grand soin lui fut apporté.', 0],
    ['Un pharmacien veilla à sa bonne conservation,', 2800],
    ['en appliquant au crâne un traitement anti-insecte et même un verni !', 5200],
    ['Mais pourquoi tant d’attentions ?', 9500],
	  ['', 12000]
  ],
  [
    ['Malgré la qualité de son traitement,', 0],
    ['la tête paraissait subir une surprenante cérémonie :', 2500],
    ['la relique était exhibée à la vue du public une fois chaque année…', 5500],
    ['Cet affront finit par atteindre les oreilles de l’Empereur Napoléon III.', 9300],
    ['Il ordonna le retour du crâne en son lieu de repos originel, en 1866.', 13000],
    ['Voilà que l’Empereur s’en mêle…', 18000],
    ['le défunt semble être d’une importance bien particulière...', 19700],
	  ['', 24000]
  ],
  [
    ['Après un long voyage de plus de 70 ans,', 0],
    ['le crâne revint finalement dans son mausolée,', 3400],
    ['prêt à reposer en paix, pour de bon.', 6200],
    ['Et quelle surprise, pour le jeune Richelieu,', 9000],
    ['de découvrir l’emplacement du tombeau.', 11500],
    ['Il s’agit de son collège…', 14300],
    ['l’endroit même où il se situe à présent.', 16400],
	  ['', 19500]
  ],
  [
    ['Et, à y regarder de plus près,', 0],
    ['le tombeau ressemble terriblement à...', 2300],
    ['celui qu’il se destine…', 4600],
    ['Il s’agit ainsi de l’histoire de la sépulture du cardinal…', 7300],
    ['Richelieu, devenu proviseur de la Sorbonne,', 11400],
    ['financera lui-même la construction de la nouvelle église du collège', 15000],
    ['où il projette d’être inhumé.', 18400],
    ['Le cardinal mourra en 1642,', 21000],
    ['mais le tombeau désiré', 24000],
    ['ne sera achevé qu’en 1694 par le sculpteur Girardon,', 26200],
    ['sur commande de la nièce de Richelieu :', 30600],
    ['Marie-Madeleine d’Aiguillon.', 33000],
    ['', 35000]
  ],
  [
    ['Le jeune Richelieu n’est pas au bout de ses surprises,', 0],
    ['car l’histoire du crâne, son... crâne,', 3700],
    ['ne semble pas s’arrêter là.', 6400],
    ['Un deuxième acte de profanation a visiblement eu lieu...', 8500],
    ['Celui-ci paraît moins violent :', 14000],
    ['un petit groupe d’individus s’active autour de la relique,', 16400],
    ['avec curiosité.', 19000],
    ['Il y a même un homme semblant observer la tête,', 21400],
    ['à travers une drôle de longue vue...', 23600],
    ['en forme de cube… ?', 26000],
    ['Cette seconde profanation eut lieu le 25 juin 1895,', 29100],
    ['sous l’impulsion de Gabriel Hanotaux, ministre des affaires étrangères.', 33700],
    ['Le groupe de curieux avait pour objectif de photographier la relique,', 38300],
    ['et d’en faire des moulages…', 41600],
    ['Une fois leur entreprise terminée,', 44300],
    ['le tombeau fut enfin refermé,', 46400],
    ['pour ne plus jamais être dérangé.', 48900],
	  ['', 52500]
  ]
]
